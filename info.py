...
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1001918797443').split()]
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
PREMIUM_USER = [int(user) if id_pattern.search(user) else user for user in environ.get('PREMIUM_USER', '').split()]
auth_channel = environ.get('AUTH_CHANNEL', '-1001918797443')
...
# MongoDB information
DATABASE_URI = environ.get('DATABASE_URI', "mongodb+srv://ILAKAHAM:pdGhsZZILAKAHAMRN7Lq2mX@anonymous.7b38ags.mongodb.net/?retryWrites=true&w=majority")
...
FILE_STORE_CHANNEL = [int(ch) for ch in (environ.get('FILE_STORE_CHANNEL', '-1001955345677')).split()]
...
URL = "https://required-ilene-kanchanmaji2004.koyeb.app".format(FQDN) if ON_HEROKU or NO_PORT else \
    "https://required-ilene-kanchanmaji2004.koyeb.app/".format(FQDN, PORT)
...
if HAS_SSL:
    URL = "https://required-ilene-kanchanmaji2004.koyeb.app/".format(FQDN)
else:
    URL = "https://required-ilene-kanchanmaji2004.koyeb.app/".format(FQDN)
