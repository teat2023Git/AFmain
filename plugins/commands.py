...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
                    ...
                    InlineKeyboardButton('🌟𝗠𝗼𝘃𝗶𝗲𝘀 𝗥𝗲𝗾𝘂𝗲𝘀𝘁 𝗚𝗿𝗼𝘂𝗽🌟', url='https://t.me/+7DcokFeX8Dk0N2Jl')
                ],[
                  InlineKeyboardButton('💋𝗔𝗱𝘂𝗹𝘁(𝟭𝟴+) 𝗠𝗼𝘃𝗶𝗲 & 𝗪𝗲𝗯𝘀𝗲𝗿𝗶𝗲𝘀 𝗖𝗵𝗮𝗻𝗻𝗲𝗹💋', url='https://t.me/+Ps2D3eRmT0BmNzY1')
                ],[
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        ...
        await client.send_photo(
            ...
            photo="https://telegra.ph/file/eea1ee9c27f4bc09039e3.jpg",
                    ...
                    reply_markup=InlineKeyboardMarkup(
                        [
                            [
                                ...
                                InlineKeyboardButton('𝗕𝗢𝗧 HELP', url="t.me/FilmyWorld2023bot")
                            ],
                            [
    ...
    if data.startswith("sendfiles"):
        ...
        k = await client.send_photo(chat_id=message.from_user.id, photo='https://telegra.ph/file/57f02a249633f225fdbf7.jpg', caption=f"<b>Get All Files in a Single Click!!!\n\n📂 ʟɪɴᴋ ➠ : {g}\n\n<i>Note: This message is deleted in 5 mins to avoid copyrights. Save the link to Somewhere else</i></b>", reply_markup=InlineKeyboardMarkup(
                [
                    [
        ...
        if settings['is_shortlink'] and user not in PREMIUM_USER:
            ...
            files = files_[0]
            g = await get_shortlink(chat_id, f"https://telegram.me/{temp.U_NAME}?start=file_{file_id}")
            k = await client.send_photo(chat_id=message.from_user.id, photo='https://telegra.ph/file/57f02a249633f225fdbf7.jpg', caption=f"<b>📕Nᴀᴍᴇ ➠ : <code>{files.file_name}</code> \n\n🔗Sɪᴢᴇ ➠ : {get_size(files.file_size)}\n\n📂Fɪʟᴇ ʟɪɴᴋ ➠ : {g}\n\n<i>Note: This message is deleted in 20 mins to avoid copyrights. Save the link to Somewhere else</i></b>", reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton('📂 Dᴏᴡɴʟᴏᴀᴅ Nᴏᴡ 📂', url=g)
                        ], [
    ...
    if not files_:
        ...
        try:
            ...
            title = '@KMCLUB2023 & @KMZone2023  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
    ...
    files = files_[0]
    title = '@KMCLUB2023 & @KMZone2023  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"@KMCLUB2023  {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))}"
    if not await check_verification(client, message.from_user.id) and VERIFY == True:
        btn = [[
            InlineKeyboardButton("Verify", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
        ]]
